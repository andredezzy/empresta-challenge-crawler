  ...
  public async execute({
    ...
    await page.evaluate(
      (findSuperiorArmyOrgan, findArmyOrgan) => {
        ...
        const ORGANS_TABLE_SUPERIOR_ARMY_ORGAN_ROW_SELECTOR =
          'td.coluna-livre.sorting_1 > span';
        const ORGANS_TABLE_ARMY_ORGAN_ROW_SELECTOR = 'td:nth-child(3) > span';
        ...
        const findRow = rows.find(row => {
          const superiorArmyOrgan = row
            .querySelector(ORGANS_TABLE_SUPERIOR_ARMY_ORGAN_ROW_SELECTOR)
            ?.textContent?.trim();

          const armyOrgan = row
            .querySelector(ORGANS_TABLE_ARMY_ORGAN_ROW_SELECTOR)
            ?.textContent?.trim();
